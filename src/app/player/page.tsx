...
export default function PlayerPage(){
 ...
 return <AppShell><PageHeader eyebrow="STREFA ZAWODNIKA" title="Centrum zawodnika"/>{gathering?<><section className="alertCard urgent"><span>!</span><div><small>NAJBLIŻSZA ZBIÓRKA · {formatDateTime(gathering.starts_at)}</small><h2>{gathering.title}</h2><p>{gathering.place}{gathering.notes?` · ${gathering.notes}`:""}</p></div></section><div className="responseBar">{options.map(([value,label])=><button key={value} className={answer===value?"active":""} onClick={()=>void respond(value)}>{label}</button>)}</div></>:<section className="alertCard"><span>✓</span><div><small>BRAK NOWYCH ZBIÓREK</small><h2>Wszystko sprawdzone</h2><p>Kapitan nie zaplanował kolejnej zbiórki.</p></div></section>}{message&&<p className="formMessage">{message}</p>}<section className="quickGrid"><Link href="/fan/lineup"><span>7/7</span><h3>Skład</h3><p>Opublikowane ustawienie</p></Link><Link href="/player/tactics"><span>06</span><h3>Taktyki</h3><p>Stałe fragmenty</p></Link><Link href="/messages"><span>!</span><h3>Komunikaty</h3><p>Wiadomości drużynowe</p></Link><Link href="/matches"><span>03</span><h3>Mecze</h3><p>Terminarz turnieju</p></Link></section></AppShell>
}
